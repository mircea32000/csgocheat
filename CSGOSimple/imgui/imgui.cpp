...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    bool ret = Begin(title,false, NULL, flags);
...
bool ImGui::BeginTransparent(const char* name, bool* p_open, ImGuiWindowFlags flags, bool visible)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			{
				// Window background
				if (!(flags & ImGuiWindowFlags_NoBackground) && !(flags & ImGuiWindowFlags_ChildWindow))
				{
					...
					auto color_grey = ImGui::GetColorU32(ImVec4(0.929, 0.929, 0.929, 1));
					auto color_black = ImGui::GetColorU32(ImVec4(0.827, 0.827, 0.827, 1));
					window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, color_grey, color_grey, color_black, color_black); // Main gradient.


					//ImGui::Button("aaaaa", ImVec2(100, 100));
				...
				if (!(flags & ImGuiWindowFlags_NoBackground) && (flags & ImGuiWindowFlags_ChildWindow))
				{
					...
					auto color_grey = ImGui::GetColorU32(ImVec4(0.929, 0.929, 0.929, 1));
					auto color_black = ImGui::GetColorU32(ImVec4(0.827, 0.827, 0.827, 1));
					window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, color_black, color_black, color_grey, color_grey); // Main gradient.
			...
			if (!(flags & ImGuiWindowFlags_NoTitleBar))
			{
				...
				window->DrawList->AddRectFilled(bottombar.Min, bottombar.Max, title_bar_col, window_rounding, ImDrawCornerFlags_BotLeft | ImDrawCornerFlags_BotRight);
				window->DrawList->AddRect(window->Pos + ImVec2(-1, window->TitleBarHeight() - 1), ImVec2(window->Pos.x - 1, window->Pos.y - 1) + ImVec2(window->Size.x + 2, window->Size.y + 1), GetColorU32(ImVec4(0, 0, 0, 1))); // Main gradient.
...
bool ImGui::Begin(const char* name, bool blackbackground, bool* p_open, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_ASSERT(name != NULL && name[0] != '\0');     // Window name required
    ...
    if (first_begin_of_the_frame)
    {
		...
		else
		{
            ...
            if (!(flags & ImGuiWindowFlags_NoBackground) && !(flags & ImGuiWindowFlags_ChildWindow))
            {
				...
				auto color_grey = ImGui::GetColorU32(ImVec4(0.929, 0.929, 0.929, 1));
				auto color_black = ImGui::GetColorU32(ImVec4(0.827, 0.827, 0.827, 1));
				auto color_greyxD = ImGui::GetColorU32(ImVec4(0.082, 0.082, 0.082, 1));
				if (blackbackground)
				{
					window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, color_greyxD, color_greyxD, color_greyxD, color_greyxD); // Main gradient.
				}
				else
				{
					window->DrawList->AddRectFilledMultiColor(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, color_grey, color_grey, color_black, color_black); // Main gradient.
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_first_use, float bg_alpha_override, ImGuiWindowFlags flags)
{
    ...
    return Begin(name,false, p_open, flags);
}
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
    ...
    bool is_open = Begin(name,false, NULL, extra_flags | ImGuiWindowFlags_Popup);
...
void ImGui::NavUpdateWindowingList()
{
    ...
    Begin("###NavWindowingList", false, NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings);
