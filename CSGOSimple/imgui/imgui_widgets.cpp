...
bool ImGui::ButtonCustomIcon(const char* label,const char* labelIcon, const ImVec2& size_arg, ImGuiButtonFlags flags, bool invertGradient)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	auto borderColor = ImColor(45, 45, 45, 255);
	auto topColor = ImColor(59, 59, 59, 255);
	auto bottomColor = ImColor(26, 26, 26, 255);
	auto bottomColorHovered = ImColor(216, 36, 36, 255);
	...
	if (!invertGradient)
	{
		window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, topColor, topColor, bottomColor, bottomColor);
		window->DrawList->AddRect(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), borderColor, 0, false, 1);
		PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, 1.0f));
		ImGui::PushFont(g_pIcons);
		RenderTextClipped(ImVec2(bb.Min.x, bb.Min.y - 12), bb.Max, labelIcon, NULL, &label_size, style.ButtonTextAlign, &bb);
		ImGui::PopFont();
		PopStyleColor();
	}
	else
	{
		window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, bottomColor, bottomColor, topColor, topColor);
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y), ImVec2(bb.Max.x, bb.Max.y + 5), bottomColorHovered, 0);
		window->DrawList->AddRect(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), borderColor, 0, false, 1);
		PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, 1.0f));
		RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
		PopStyleColor();
...
bool ImGui::ButtonCustom(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool invertGradient)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	auto bottomColorHovered = ImColor(25, 46, 46, 255);
...
{
	return ButtonCustom(label, size_arg, 0, false);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	RenderFrame(ImVec2(check_bb.Min.x, check_bb.Min.y + 2), ImVec2(check_bb.Max.x, check_bb.Max.y + 2), borderColor, true, style.FrameRounding);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	{
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 3), ImVec2(frame_bb.Max.x, frame_bb.Max.y + 3), GetColorU32(ImVec4(0, 0, 0, 1)), style.FrameRounding, ImDrawCornerFlags_All);
    ...
    bool ret = Begin(name, false, NULL, window_flags);
    PopStyleVar();
    if (!ret)
    {
        EndPopup();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f));
    const ImRect total_bb(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 10));
	...
	//if (label_size.x > 0.0f)
	//	RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 11), label);
		//Text(label);
		//   RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 11), label);
	...
	RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 7), ImVec2(frame_bb.Max.x - 1, frame_bb.Min.y + h / 2 + 7), GetColorU32(ImVec4(0, 0, 0, 1)), true, g.Style.FrameRounding);
	RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5), ImVec2(frame_bb.Max.x, frame_bb.Min.y + h / 2 + 5), frame_col, true, g.Style.FrameRounding);
    ...
    // Render grab
	window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + 3), ImVec2(grab_bb.Max.x, grab_bb.Max.y + 3), GetColorU32(ImVec4(0, 0, 0, 1)), style.GrabRounding);
    window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
	window->DrawList->AddRect(ImVec2(grab_bb.Min.x - 1, grab_bb.Min.y - 1), ImVec2(grab_bb.Max.x + 1, grab_bb.Max.y + 1), GetColorU32(ImVec4(0.737f, 0.737f, 0.768f,1.f)), style.GrabRounding);
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
	PushFont(g_pDefaultFontSmall);
    RenderTextClipped(ImVec2(frame_bb.Min.x,frame_bb.Min.y + 12), ImVec2(frame_bb.Max.x, frame_bb.Max.y + 12), value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));
	PopFont();
	RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
