...
bool ImGui::BeginGroupBox(const char* name, const ImVec2& size_arg)
{
	...
	bool ret;
	ImGui::Begin(name, false, &ret, flags);
...
bool ImGui::BeginGroupBoxInvisible(const char* name, const ImVec2& size_arg, bool visible)
{
	ImGuiWindow* window = ImGui::GetCurrentWindow();
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow;
	...
	ImGui::SetNextWindowSize(size);
	bool ret;
	ImGui::BeginTransparent(name, &ret, flags, visible);
	//bool ret = ImGui::Begin(name, NULL, size, -1.0f, flags);
